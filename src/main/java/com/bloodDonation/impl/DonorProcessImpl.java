...
public class DonorProcessImpl {
  ...
  public void createDonor(Donor donor) {
    try {
      Connection con = dbConnector.getConnection(mapper.getUrl(),
          mapper.getRoot(), mapper.getPassword(),
          new HashMap<>());
      ...
      if (con != null) {
        ...
        String sql = "INSERT INTO donor (username, first_name, last_name, password, gender, date_of_birth, blood_group, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
// Remember to close the PreparedStatement and Connection objects when done
      ...
      throw new GenericException("An error occurred while connecting to the database: " + ex.getMessage());
    }
  }
    ...
    if(con!=null){
      ...
      try {
        ...
        if (rs.next()) {
          ...
          throw new GenericException("User details not found please try to login with correct details");
        }
      } catch (SQLException e) {
        throw new GenericException(e.getMessage());
      }
  ...
  public ContestResponse getContest(String userName){
    ...
    if (con != null) {
      String sqlJoined = "SELECT contest_id, name, description FROM contest WHERE contest_id in (select contest_id from contest_donation where donor_username = ?)";
      String sqlNotJoined = "SELECT contest_id, name, description FROM contest WHERE contest_id NOT IN (select contest_id from contest_donation where donor_username=? and contest_id is not null)";
      try {
  ...
  public AppointmentResponse getAppointment(String donorUsername) {
    ...
    if(con!=null) {
      String sql = "SELECT * FROM appointment WHERE appointment_for =? and appointment_time >= Now() and checked_in = false order by appointment_time";
      PreparedStatement stmt = null;
      try {
        stmt = con.prepareStatement(sql);
  ...
  public AppointmentResponse createAppointment(CreateAppointment createAppointmentRequest) {
    try {
      ...
      if (con != null) {
        String sql = "INSERT INTO appointment (appointment_time, appointment_with, appointment_for,contest_id) VALUES (?, ?, ?,?)";
        // Set the parameter values for the query
        try {
          PreparedStatement stmt = con.prepareStatement(sql);
  ...
  public AppointmentResponse deleteAppointment(DeleteAppointmentRequest deleteAppointmentRequest) {
    ...
    AppointmentResponse appointmentResponse = new AppointmentResponse();
    try {
     con = dbConnector.getConnection(mapper.getUrl(),
          mapper.getRoot(), mapper.getPassword(),
          new HashMap<>());
      if (con != null) {
  ...
  public DonationResponse getDonations(String donorsUserName) {
    ...
    if(con!=null) {
      String sql = "SELECT * FROM appointment WHERE appointment_id IN (select appointment_id from appointment where appointment_for = ?) order by appointment_time DESC";
      PreparedStatement stmt = null;
      try {
        stmt = con.prepareStatement(sql);
