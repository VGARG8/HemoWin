...
// make the HTTP request and deserialize the response body to DonorResponse object
          ResponseEntity<WorkerResponse> response = restTemplate.postForEntity(
          ...
          if (response.getStatusCode() == HttpStatus.OK && !workerResponse.getStatus().equals("FAILURE")) {
//            // Switch to the appropriate panel
            cardPanel.add(new WorkerMainAppPage(cardPanel, cardLayout, workerResponse.getWorker()), "worker-main-app");
            cardLayout.show(cardPanel, "worker-main-app");
          } else {
            JOptionPane.showMessageDialog(new JOptionPane(), workerResponse.getMessage(), "Error",
                JOptionPane.ERROR_MESSAGE);
          }
        }
      }
