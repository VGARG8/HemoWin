        ...
        submit.addActionListener(new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            try {
              ...
              cardPanel.add(new MainAppPage(cardPanel, cardLayout, donorResponse.getDonor()),
                  "worker-main-page");
              cardLayout.show(cardPanel, "worker-main-page");
            }catch (HttpClientErrorException ex){
              JOptionPane.showMessageDialog(new JOptionPane(),
                  "Error in connecting with server please try after some time", "Error",
...
// make the HTTP request and deserialize the response body to ContestResponse object
      ...
      ResponseEntity<ContestResponse> response = restTemplate.exchange(url, HttpMethod.GET,
          requestEntity, ContestResponse.class);
      ...
      for (Contest contest : contestResponse.getActiveContest()) {
        ...
        activeContest.addActionListener(new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            /**
             * Registering contest.
...
// create HTTP entity with headers
              HttpEntity<JoinContestRequest> requestEntity = new HttpEntity<>(joinContestRequest,
                  headers);

// make the HTTP request and deserialize the response body to ContestResponse object
              String url = "http://localhost:8080/donor/contest";
              ResponseEntity<ContestResponse> response = restTemplate.postForEntity(
                  url, requestEntity, ContestResponse.class);

              cardPanel.add(new MainAppPage(cardPanel, cardLayout, donor),
                  "appointment-form");
      ...
      if(! contestResponse.getJoinedContest().isEmpty()){
        ...
        bookAppointment.addActionListener(new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent e) {
            cardPanel.add(new AppointmentPanel(cardPanel,cardLayout,donor, contestResponse.getJoinedContest()),"appointment-form");
            cardLayout.show(cardPanel,"appointment-form");
          }
        });
      }
...
// make the HTTP request and deserialize the response body to ContestResponse object
    try {
      ...
      ResponseEntity<AppointmentResponse> response = restTemplate.exchange(url, HttpMethod.GET,
          requestEntity, AppointmentResponse.class);
...
// set properties of the request object as needed
      ...
      ResponseEntity<LeaderBoard> leaderBoardResponse = leaderboardRestTemplate.exchange(
